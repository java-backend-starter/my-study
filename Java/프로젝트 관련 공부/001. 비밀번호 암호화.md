...
- 특징
  - Deterministic : 같은 입력은 언제나 같은 출력을 생성한다. 
  - 고정 길이 출력 : 입력 크기와 관계없이 결과값의 길이는 항상 동일하다. 
  - 용도 : 메시지 인증, 데이터 무결성 검증 등에서 활용된다. 
  - 충돌 가능성 존재 : 서로 다른 입력이 동일한 해시값을 가질 수 있다. 이를 해시 충돌(Hash Collision)이라고 한다. 
  - 단방향성 : 해시값만으로 원래 입력 데이터를 복원할 수 없다. 
  - 고속 처리 : 본질적으로 매우 빠르게 계산되기 때문에, 공격자가 브루트포스 시도를 하기에도 유리한 면이 있다. 
  - 결과값 명칭 : 해시 결과값은 digest, checksum, digital fingerprint 등으로 불린다. 
  - 대표적인 알고리즘 예시 : SHA256, SHA512, MD5 등이 있다.

### 단방향 암호화 사용 이유

- 서버 DB에 실제 비밀번호 저장하지 않는다. → 해킹 위험 감소
- 로그인 시 비교 방식:
...
- 너무 빠르다
  ...
  - 예: MD5는 일반 PC에서도 1초에 수십억 번 추론할 수 있다.
...
- SHA256은 256비트라 충돌 가능성은 매우 낮지만, 완전히 불가능하지 않다.

---

## 4. 해시 함수 구현 : MessageDigest 알고리즘

- MessageDigest는 다양한 해시 알고리즘을 지원하고 SHA-256 같은 표준 해시를 쉽게 사용할 수 있게 하는 인터페이스이다.
...
- 보통 0.2초 이상 걸리도록 반복 횟수를 조절하는 것이 권장된다.
- Salt와 함께 사용한다.
...
- 장점:
    - 같은 비밀번호라도 다른 사용자는 서로 다른 해시값을 저장한다.
    - 사전 공격이나 레인보우 테이블 공격을 무력화한다.
