...
- 구조: 내부적으로 Red-Black Tree라는 정렬된 이진 탐색 트리 구조 사용 
- 장점: 저장과 동시에 정렬 상태 유지 
  - 오름차순 정렬이 기본이며, Comparator를 사용하면 사용자 정의 정렬 가능 
- 단점: 트리 구조이므로 연산 성능은 O(log n) 수준 
  - HashSet/LinkedHashSet보다 느림 
- 중복 판단: compareTo()나 Comparator.compare()의 결과가 0이면 중복으로 판단 
  - equals()와 기준이 다를 수 있음
- 스레드 안전성: 비동기
- 추천 환경: 데이터를 정렬된 형태로 유지하면서, 범위 기반 검색(subSet(), headSet(), tailSet())이 필요한 경우
```java
...
- subSet(fromElement, toElement)
  - fromElement 이상 toElement 미만의 범위 반환 
  - 예: set.subSet(10, 20) → `[10, 20)`
- headSet(toElement)
  - toElement 미만의 모든 요소 반환 
  - 예: set.headSet(30) → `[최솟값, 30)`
- tailSet(fromElement):
  - fromElement 이상의 모든 요소 반환 
  - 예: set.tailSet(40) → `[40, 최대값]`
```java
SortedSet<Integer> sub = treeSet.subSet(10, 20);
SortedSet<Integer> head = treeSet.headSet(30);
SortedSet<Integer> tail = treeSet.tailSet(40);
```
...
## 10. HashSet vs HashMap vs Hashtable vs Properties 비교
