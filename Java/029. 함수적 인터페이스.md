...
| BiConsumer<T, U>       | accept(T t, U u)       | 두 객체 소비            |
...
```
Objects.equals(source, target) 반환 값
sourceObject	targetObject	리턴값
null	null	true
not null	null	false
null	not null	false
not null	not null	source.equals(target) 결과
Ⅷ. minBy(), maxBy() 정적 메소드
BinaryOperator<T>는 두 값을 비교해서 최소 또는 최대값을 반환하는 다음의 정적 메소드를 제공합니다:
