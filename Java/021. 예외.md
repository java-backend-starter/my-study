...
- 하드웨어 오동작이나 고장 등으로 발생하는 프로그램 실행 오류
- JVM 실행하는 과정에서 발생하는 문제이므로 프로그램을 견고하게 만들어도 처리할 수 없음
- 예: OutOfMemoryError, StackOverflowError

### 예외(Exception)

- 사용자 조작 실수나 개발자의 잘못된 코딩으로 발생하는 프로그램 실행 오류
- 예외 처리(Exception Handling)로 프로그램의 정상 실행 유지 가능
- 모든 예외는 객체이며, Exception 클래스를 상속받아 생성된다
